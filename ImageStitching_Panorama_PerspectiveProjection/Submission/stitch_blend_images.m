...
% 10 high matches
% matches_high = matches(:,scores_ind(1:200));
...
blend_left_ind = LeftCol;
blend_right_ind = sl2;
if to_left == 1
    Ib1 = IM(rsz-rsz/8:rsz+rsz/8,blend_left_ind:blend_right_ind,1:3);
%     Ibf1 = [Ib1 Ib1];
%     Ib2 = IM(:,LeftCol:blend_right_ind,1:3);
%     Ibf2 = [Ib2 Ib2];
elseif to_left == 0
    IM = plane0(1:max(sr1,BottomRow),min(plane_size-sr2,plane_size-sr2+LeftCol):end,1:3);
end

 [bl_m,wid] = blendalp(Ib1,Ib1);
 IM_b = IM;
 IM_b(rsz-rsz/8:rsz+rsz/8,blend_left_ind:blend_left_ind+wid-1,:)=bl_m;
 figure, imshow(IM_b); title 'blended image'
