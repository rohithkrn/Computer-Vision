...
I1 = imresize(im2double(imread('hand.jpg')),[250 250]);
I2 = imresize(im2double(imread('eye.jpg')),[250 250]);

[s1,s2,s3] = size(I1);

% c = [151 136 136 150 165 172 166];
% r = [111 125 150 167 151 133 114];

c = [177 157 154 164 175 192 193 189];
r = [138 150 179 202 205 191 175 150];
...
G1_pyr =  getGaussPyr(BWc,X,numlevels);


% G2 = zeros(s1,s2);
% G2(:,s1/2:end) = 1;
% G2_pyr =  getGaussPyr(G2,X,numlevels);
G2_pyr =  getGaussPyr(BW,X,numlevels);
...
% 
% figure,subplot(3,3,1),imshow(G1_pyr{1}); title 'Level 1 - Gaussian Mask1 Pyr';
% subplot(3,3,2),imshow(G1_pyr{2}); title 'Level 2 - Gaussian Mask1 Pyr';
% subplot(3,3,3),imshow(G1_pyr{3}); title 'Level 3 - Gaussian Mask1 Pyr';
% subplot(3,3,4),imshow(G1_pyr{4}); title 'Level 4 - Gaussian Mask1 Pyr'
% subplot(3,3,5),imshow(G1_pyr{5}); title 'Level 5 - Gaussian Mask1 Pyr'
% subplot(3,3,6),imshow(G1_pyr{6}); title 'Level 6 - Gaussian Mask1 Pyr'
% 
